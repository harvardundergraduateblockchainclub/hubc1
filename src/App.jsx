...
const InitiativeCard = ({ n, status, title, children }) => (
  <div className="hubc-card" style={{ background: '#fff', padding: 40 }}>
    ...
    <p style={{ fontSize: 15, lineHeight: 1.7, color: 'rgba(23,20,15,0.6)', margin: 0 }}>{children}</p>
  </div>
);
    ...
    <div style={{ fontFamily: SANS, color: INK, background: PAPER, minHeight: '100vh' }}>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; }
        ...
        .hubc-btn-dark:hover { background: ${CRIMSON} !important; }
        .hubc-btn-ghost:hover { border-color: ${INK} !important; }
        .hubc-btn-outline:hover { background: ${INK} !important; color: ${PAPER} !important; }
        .hubc-btn-solid:hover { background: ${INK} !important; }
        .hubc-card { transition: background 0.3s ease; }
        .hubc-card:hover { background: #FFFCFC; }
        .hubc-portrait { filter: grayscale(1); transition: filter 0.4s ease; }
        .hubc-portrait:hover { filter: grayscale(0); }
        .hubc-foot a:hover { color: ${CRIMSON} !important; }
      ...
      <nav
        ...
        style={{
          ...
          padding: '18px 56px', background: 'rgba(250,250,248,0.82)',
        ...
        <section>
          ...
          >
            ...
            <div style={{ position: 'relative', zIndex: 2, maxWidth: 1240, margin: '0 auto', width: '100%' }}>
              <div style={{ fontFamily: MONO, fontSize: 12, letterSpacing: '0.18em', textTransform: 'uppercase', color: CRIMSON, marginBottom: 28, animation: 'hubcFadeUp 0.8s ease both' }}>
          ...
          <div className="hubc-pad" style={{ padding: '120px 56px', background: '#fff', borderTop: '1px solid rgba(23,20,15,0.08)' }}>
            <div className="hubc-two-col" style={{ maxWidth: 1240, margin: '0 auto', display: 'grid', gridTemplateColumns: 'minmax(0, 0.9fr) minmax(0, 1.1fr)', gap: 80, alignItems: 'start' }}>
              <h2 style={{ fontFamily: SERIF, fontWeight: 400, fontSize: 'clamp(34px, 4vw, 56px)', lineHeight: 1.06, letterSpacing: '-0.015em', margin: 0 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 22 }}>
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10, marginTop: 8 }}>
                  {['Research', 'Engineering', 'Policy', 'Industry'].map((t) => (
                    <span key={t} style={{ fontFamily: MONO, fontSize: 11, letterSpacing: '0.1em', textTransform: 'uppercase', padding: '8px 14px', border: '1px solid rgba(23,20,15,0.16)', borderRadius: 2, color: 'rgba(23,20,15,0.6)' }}>
                      {t}
                    </span>
                  ))}
                </div>
        ...
        <section className="hubc-pad" style={{ padding: '160px 56px 120px' }}>
          <div style={{ maxWidth: 1240, margin: '0 auto' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 1, background: 'rgba(23,20,15,0.1)', border: '1px solid rgba(23,20,15,0.1)' }}>
              <InitiativeCard n="01" status="Active" title="Blockchain & AI Summit 2026">
      ...
      <footer className="hubc-foot hubc-pad" style={{ borderTop: '1px solid rgba(23,20,15,0.08)', padding: '32px 56px', background: '#fff' }}>
        <div style={{ maxWidth: 1240, margin: '0 auto', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 24, flexWrap: 'wrap' }}>
          ...
          <div style={{ display: 'flex', gap: 26 }}>
            {[['X', 'https://x.com'], ['Instagram', 'https://instagram.com'], ['Email', EMAIL]].map(([label, href]) => (
              <a key={label} href={href} style={{ fontFamily: MONO, fontSize: 11, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'rgba(23,20,15,0.5)' }}>
                {label}
              </a>
            ))}
          </div>
